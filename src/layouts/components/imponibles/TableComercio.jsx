...
const TableComercio = () => {
    ...
    useEffect(() => {
        if (sistemas && sistemas.rafam_imponibles.comercios) {
            setData(sistemas.rafam_imponibles.comercios);
        }
    }, []);
    ...
    const updateImponible = (e) => {
        ...
        const results = data.filter((item) =>
            item.NRO_COMERCIO.toString().includes(value)
        );
        setFilteredResults(results);
        if (value == "")
            setFilteredResults(sistemas.rafam_imponibles.comercios);
    };
                ...
                paginatedPages.length > 0 ? (
                    <>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                            {paginatedPages.map(({ NRO_COMERCIO }, index) => (
                                <Link
                                    key={index}
